    ...
    ws.on('message', async (data) => {
      ...
      try {
        switch (msg.action) {
          ...
          case 'load_session': { const s = tabs.get(tabId); if (s) { tabs.delete(tabId); teardown(s); } try { const ns = await createSession(ws, msg.sessionId, tabId); tabs.set(tabId, ns); watchProc(ns, tabId); } catch (e: any) { console.error(`[load_session] sessionId=${msg.sessionId}:`, e); try { const ns = await createSession(ws, undefined, tabId); tabs.set(tabId, ns); watchProc(ns, tabId); } catch { return; } const detail = e.code ? `${e.message} (${e.code})` : e.data ? `${e.message}: ${JSON.stringify(e.data)}` : e.message; emit(ws, { type: 'error', tabId, message: `Failed to load session ${msg.sessionId?.slice(0, 8)}: ${detail}` }); } emit(ws, { type: 'ready', tabId, sessionId: tabs.get(tabId)!.sessionId, modes: tabs.get(tabId)!.modes, models: tabs.get(tabId)!.models }); break; }
        ...
        console.error(`Error in action=${msg.action} tabId=${tabId}:`, e.message, e.code || '', e.data || '');
