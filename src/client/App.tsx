...
export function App() {
  ...
  // Forward command option updates to ChatInput component via window ref
  const setCmdFilter = useCallback((v: CommandOption[] | null) => {
    const setter = (window as unknown as Record<string, unknown>).__chatInputSetCmdFilter as ((options: CommandOption[] | null | undefined, hint?: string | null) => void) | undefined;
    if (setter) setter(v, undefined);
  }, []);
  const setCmdIdx = useCallback((_v: number) => {}, []);
  const setCmdHint = useCallback((v: string | null) => {
    const setter = (window as unknown as Record<string, unknown>).__chatInputSetCmdFilter as ((options: CommandOption[] | null | undefined, hint?: string | null) => void) | undefined;
    if (setter) setter(undefined, v);
  }, []);
  ...
  // Auto-scroll — use instant during streaming to avoid smooth animation fights
  useEffect(() => {
    const el = messagesContainerRef.current;
    if (!el) return;
    const isNearBottom = el.scrollHeight - el.scrollTop - el.clientHeight < 100;
    if (isNearBottom) {
      messagesEndRef.current?.scrollIntoView({ behavior: tab.isRunning ? 'instant' : 'smooth' });
    }
  }, [tab.messages, tab.thinking, tab.isRunning]);
  ...
  // Highlight code blocks — skip during streaming to prevent layout thrashing
  const hljsTimeoutRef = useRef<ReturnType<typeof setTimeout>>(undefined);
  useEffect(() => {
    clearTimeout(hljsTimeoutRef.current);
    // During streaming, debounce heavily; after streaming completes, run immediately
    const delay = tab.isRunning ? 500 : 0;
    hljsTimeoutRef.current = setTimeout(() => {
    ...
    return () => clearTimeout(hljsTimeoutRef.current);
  }, [tab.messages, tab.isRunning]);
  ...
  function handleSend(text: string, images?: PendingImage[], files?: PendingFile[]) {
    if (!text) return;
    if (text.startsWith('/')) {
      ...
      if (panelType) { updateTab(activeTabId, t => ({ ...t, messages: [...t.messages, { role: 'user', text }, { role: 'panel', text: '', panelType }] })); return; }
      if (tab.isRunning) { updateTab(activeTabId, t => ({ ...t, queue: [...t.queue, text] })); return; }
    } else {
      if (tab.isRunning) { updateTab(activeTabId, t => ({ ...t, queue: [...t.queue, text] })); return; }
    }
    ...
    send({ action: 'prompt', tabId: activeTabId, text, images: images || undefined, files: files || undefined });
  }
  ...
  return <AppContext.Provider value={ctx}><ErrorBoundary><>
    ...
    <div className="main">
      ...
      <ChatInput
        ...
        onCancel={() => send({ action: 'cancel', tabId: activeTabId })}
        onGoalCancel={() => { updateTab(activeTabId, t => ({ ...t, goal: null })); send({ action: 'prompt', tabId: activeTabId, text: '/goal clear' }); }}
        onGoalDismiss={() => updateTab(activeTabId, t => ({ ...t, goal: null }))}
        onQueueUpdate={(fn) => updateTab(activeTabId, fn)}
        onCommandOptions={(command, input) => send({ action: 'command_options', tabId: activeTabId, command, input })}
        onSetCommandFilter={() => {}}
        sendWs={send}
      />
    </>}
